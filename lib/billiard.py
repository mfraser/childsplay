...
class Ball(SPSpriteUtils.SPSprite):
    ...
    def __init__(self, pic, start, kind):
        ...
        SPSpriteUtils.SPSprite.__init__(self, self.image,
                                        name=kind)  # embed it in this class, eg make SPSpriteUtils.SPSprite part of Ball
    ...
    def madehole(self):
        ...
        diff = (self.posreal[0] - Hole.pos[0] + 30, self.posreal[1] - Hole.pos[1] + 30)
...
class Stick(SPSpriteUtils.SPSprite):
    ...
    def grow(self, scale):
        ...
        self.rect = self.pic.get_rect().move(self.positmouse[0] + int(self.disp[0] * sc),
                                             self.positmouse[1] + int(self.disp[1] * sc))
        Misc.group.refresh()
...
class Activity:
  ...
  """
                ...
                if event.type is MOUSEBUTTONDOWN:
                    ...
                    if event.button >= 2:
                        ...
                        if self.ball1.callback(event.pos, event.button):
                            ...
                            self.stick.prepare(event.pos, (self.ball1.posreal[0] + self.ball1.size[0] / 2,
                                                           self.ball1.posreal[1] + self.ball1.size[1] / 2))
    ...
    def next_exercise(self):
        ...
        objects = []
        self.screen.blit(self.backgr_wohole, (
        0, self.blit_pos[1]))  # this is needed here, as CP calls the start (and not setup) proc. on a level change.
        x, y = (random.randrange(8, 780 - Hole.size[0], 1),
                random.randrange(self.blit_pos[1] + 50, 600 - self.blit_pos[1] - Hole.size[1], 1))
        Hole.pos = (x, y)
        ...
        x, y = (random.randrange(8, 780 - imgsize[0], 1), \
                random.randrange(self.blit_pos[1] + 50, 491 - self.blit_pos[1] - imgsize[1], 1))
        while self.tooclose((x, y, imgsize), objects):
            x, y = (random.randrange(8, 791 - imgsize[0], 1), \
                    random.randrange(self.blit_pos[1] + 50, 491 - self.blit_pos[1] - imgsize[1], 1))
        objects.append([(x, y), imgsize])
        self.ball1 = Ball(img, (x, y),
                          'blue')  # the extra argument can be 'blue' or 'red'. this way we know what kind of ball it is.
        ...
        if self.level == 2:
            ...
            self.pointstoadd = 0.2 * diff1norm + (diff1[0] * diff2[0] + diff1[1] * diff2[1]) / (
            6 * diff1norm) + diff2norm * 0.3
        if self.level == 3:
            img = utils.load_image(os.path.join(self.my_datadir, 'ball2.png'))
            imgsize = (img.get_width(), img.get_height())
            x, y = (random.randrange(8, 791 - imgsize[0], 1), random.randrange(8, 491 - imgsize[1], 1))
        ...
        if (len(Misc.group) == (self.level > 1)):  # true if there are no balls in level 1 or there's in levels 2 and 3
            ...
            if self.tries == self.n_balls:
                self.SPG.tellcore_level_end(
                    store_db=True)  #signal to childsplay that this level is ended. See return val of def loop
                if self.level == 3:
                    self.stop = 1
